...
const load = () => fireEvent(screen.getByTestId('viewer-image'), 'load', { nativeEvent: { source: { width: 1200, height: 800 } } });
describe('사진 전체화면', () => {
  it('누른 배열 index로 열고 원 URI 하나만 표시한다', () => {
    renderWithTheme(<PhotoViewer {...props} />);
    expect(screen.getByText('2 / 2')).toBeTruthy();
    expect(screen.getByTestId('viewer-image').props.source).toEqual({ uri: photos[1].uri });
    expect(screen.getAllByTestId('viewer-image')).toHaveLength(1);
  });
  ...
  it('오류 후에도 이동 가능하고 닫을 수 있다', async () => {
    renderWithTheme(<PhotoViewer {...props} />);
    fireEvent(screen.getByTestId('viewer-image'), 'error');
