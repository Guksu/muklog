...
describe('parseMapMessage', () => {
  ...
  it('MARKER_TAP(id) 메시지를 파싱한다', () => {
    expect(parseMapMessage({ raw: JSON.stringify({ type: 'MARKER_TAP', id: 'm9' }) })).toEqual({
  ...
  it('MARKER_TAP에 id가 없으면 null을 반환한다(필드 누락 방어)', () => {
    expect(parseMapMessage({ raw: JSON.stringify({ type: 'MARKER_TAP' }) })).toBeNull();
  });
