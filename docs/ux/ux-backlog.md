...
| U8 | 에디터 | 저장 실패 메시지가 폼 최하단에만 렌더 — 저장은 상단 SubBar(`MuklogEditor.tsx:409-427`), 에러는 방문일 아래(`:636-640`) → 스크롤 위치 따라 화면 밖, 탭해도 무반응처럼 보임 | 3 | 실패 시 전역 토스트 + 인라인 병행(스크롤안 기각 D3) — editor-fidelity "실패 토스트 없음" 계약 정식 개정 | 상 | 하 | 없음 | 완료(silent-failure-feedback, docs/history/2026-09-06-silent-failure-feedback.md — 실기기 M2 관찰 대기) |
