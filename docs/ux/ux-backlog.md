...
| U56 | 먹로그 상세 | 사진을 탭해도 아무 일 없음 — 원본 크게 볼 수 없음(사용자 직접 요청 2026-09-04 ①) | 1 | 사진 탭 → 풀스크린 원본 슬라이드 뷰어(가로 스와이프·닫기). 핀치줌은 reanimated 필요라 2단계 이월 가능 | 상 | 중 | 없음(킷은 상세 뷰어 침묵) | 대기 |
| U57 | 먹로그 상세 | 시트/다이얼로그 딤이 네비게이션(헤더) 영역까지 덮지 않음(사용자 직접 요청 2026-09-04 ②) | 4 | 딤 레이어가 화면 전체(헤더 포함)를 덮게 — Modal 기반 or 풀스크린 오버레이로 정합 | 중 | 하 | 없음 | 대기 |
| U58 | 전역 | 편집 후 상세/리스트 복귀 시 이미지 깜빡 — 화면마다 재조회로 캐시 부재(사용자 직접 요청 2026-09-04 ③) | 3 | **tanstack-query 도입**으로 전역 API 캐시·무효화 체계 전환(아키텍처 변경 — 설계 문답 필요, 별도 대형 스프린트) | 상 | 상 | 없음 | 대기 |
| U59 | 위시리스트 | 위시 목록에 '다녀왔어요' 칩이 보임 — 위시는 아직 안 간 곳이라 정보 모순(사용자 직접 요청 2026-09-04 ④: 없애도 될 듯) | 6 | 위시리스트 카드에서 방문 칩 제거(조건 렌더) | 중 | 하 | 없음(킷 대조 필요 시 확인) | 대기 |
