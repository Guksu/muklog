...
| U6 | 위시리스트 | 위시 추가 실패가 완전 무음 — `LogScreen.tsx:246-247` catch 공백, 검색뷰만 닫힘(`:251`) → "담았는데 목록에 없다". 진행 표시도 없음(`:227-252`) | 3·7 | catch에서 `mapWishlistError` 토스트(지도 담기와 통일) + 제출 중 행 비활성/진행 표시 + **실패 시 검색뷰 유지(D2)** | 상 | 하 | 없음 | 완료(silent-failure-feedback, docs/history/2026-09-06-silent-failure-feedback.md — 실기기 M1·M3 관찰 대기) |
...
| U8 | 에디터 | 저장 실패 메시지가 폼 최하단에만 렌더 — 저장은 상단 SubBar(`MuklogEditor.tsx:409-427`), 에러는 방문일 아래(`:636-640`) → 스크롤 위치 따라 화면 밖, 탭해도 무반응처럼 보임 | 3 | 실패 시 전역 토스트(기존 `useToastController`) + 인라인 유지(또는 에러 위치로 스크롤) | 상 | 하 | 없음 | 완료(editor-trust, 2026-09-15 · 미출시; 네이티브 제스처 추가 확인 필요) |
| U9 | 에디터 | 메모 5자 필수가 매 기록 키보드 입력 강제 — `MuklogEditor.tsx:320-321`. 킷은 메모 선택(`mk-log.jsx:451` required 없음), 별점 필수(`mk-log.jsx:390`, `SPEC.md:139`) | 2 | 킷 계약 복귀(메모 선택 + 별점 필수) 또는 현행 유지·완화 — **필수 정책 사용자 결정 필요** | 상 | 하 | ⚠️ 사용자 확인 필요 | 완료(editor-trust, 2026-09-15 · 미출시; 네이티브 제스처 추가 확인 필요) |
...
| U56 | 먹로그 상세 | 사진 전체화면 1차 구현 이후 확대 기능 추가 | 1 | 기존 전체화면·배열 인덱스·접근성 계약에 1~3배 핀치/버튼 확대와 확대 사진 이동을 통합. 기존 RNGH/Animated 사용으로 새 네이티브 의존성 없음. U56-b 끌어내려 닫기·U56-c 종료 인덱스 역동기화는 이월 | 상 | 중 | 없음(킷은 상세 뷰어 침묵) | 1차 완료(docs/history/2026-09-05-photo-viewer.md), 확대 통합 완료(PR #30, 231 suites/2637 tests 통과, docs/history/2026-09-21-photo-viewer.md). 실제 다중터치·Android 검증 남음 |
| U57 | 먹로그 상세 | 시트/다이얼로그 딤이 네비게이션(헤더) 영역까지 덮지 않음(사용자 직접 요청 2026-09-04 ②). **원인 확정(2026-09-05)**: Android RN Modal의 `statusBarTranslucent` 부재 — 상태바 띠 미커버(프로덕션 Modal 4곳 전부) | 4 | Modal 4곳 `statusBarTranslucent` + RenameDialog 상단 보정(`resolveModalTopInset`). 하단 시스템 내비바는 RN 0.76 한계로 이월(0.77+ `navigationBarTranslucent` 1줄) | 중 | 하 | 없음(킷 복귀) | 완료(dim-full-cover, docs/history/2026-09-05-dim-full-cover.md — 실기기 스모크 DS1~DS9 관찰 대기) |
| U58 | 전역 | 편집 후 상세/리스트 복귀 시 이미지 깜빡 — ① 복귀마다 재조회(캐시 부재) ② 재조회마다 서명 URL 재발급(이미지 캐시 미스)(사용자 직접 요청 2026-09-04 ③) | 3 | 설계 문답 완료(2026-09-05, D1~D4): tanstack-query+URL 재사용, 캐시 먼저+백그라운드 갱신, 어댑터로 계약 유지. **1차 완료**: 증상 경로 훅 3개(useMuklogs·useMuklog·useMyLogs)+서명 URL 캐시. **2차 이월**: 나머지 useOneShotQuery 훅 9개 순차 전환 | 상 | 상 | 없음 | 진행(1차 완료 query-cache, docs/history/2026-09-05-query-cache.md — 2차 9훅 전환·실기기 스모크 DS1~DS8 대기) |
| U59 | 위시리스트 | 위시 목록에 '다녀왔어요' 칩이 보임 — 위시는 아직 안 간 곳이라 정보 모순(사용자 직접 요청 2026-09-04 ④: 없애도 될 듯). **진단 정정(2026-09-05)**: 칩이 아니라 위시→기록 전환 유일 진입점인 액션 버튼(`WishlistView.tsx` onVisit) — 실제 위반은 원칙 5(상태 서술 카피가 액션에 붙음) | 5 | ~~방문 칩 제거~~ → **사용자 결정(2026-09-05): 버튼 유지, 라벨만 "기록하기"로**(기능 손실 0). 킷 `ex.visitBtn(232)` 카피 이탈은 사용자 승인(K1, 킷 확보 시 역반영) | 중 | 하 | ⚠️ 승인됨(K1) | 완료(wish-visit-pill, docs/history/2026-09-05-wish-visit-pill.md) |
...
| U16 | 에디터 | 필수 표시↔실제 검증 불일치 — 장소만 `*`(`MuklogEditor.tsx:464-466`), 메모는 표시 없이 차단(`:320-321`, `validate.ts:10`), 킷 필수인 별점은 둘 다 없음(`:575-577`). 저장 비활성 사유 미표시 | 2·5 | U9의 필수 정책 확정 후 `*` 표기·검증 일치 + 비활성 시 1줄 안내("장소와 메모를 채우면 저장할 수 있어요") | 상 | 중 | ⚠️ U9에 종속 | 완료(editor-trust, 2026-09-15 · 미출시; 네이티브 제스처 추가 확인 필요) |
...
| U43 | 에디터 | 뒤로가기 시 입력 무경고 소실(`MuklogEditorRoute.tsx:75`,`:117`) | 2·7 | dirty 판정 → 확인 시트("작성 중인 기록을 지울까요?") | 중 | 중 | 없음 | 완료(editor-trust, 2026-09-15 · 미출시; 네이티브 제스처 추가 확인 필요) |
