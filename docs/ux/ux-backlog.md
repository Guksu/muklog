...
| U56 | 먹로그 상세 | 사진을 탭해도 아무 일 없음 — 원본 크게 볼 수 없음(사용자 직접 요청 2026-09-04 ①) | 1 | 사진 탭 → 풀스크린 원본 슬라이드 뷰어(가로 스와이프·닫기). ~~핀치줌은 reanimated 필요라 2단계 이월 가능~~ → 이월 확정 3종: U56-a 핀치줌(reanimated=네이티브 릴리스, 사용자 결정 대기)·U56-b 끌어내려 닫기·U56-c 종료 인덱스 역동기화 | 상 | 중 | 없음(킷은 상세 뷰어 침묵) | 완료(photo-viewer, docs/history/2026-09-05-photo-viewer.md — 실기기 스모크 DS1~DS8 관찰 대기) |
