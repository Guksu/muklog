...
| 1라운드 이슈 L1~L4 | **전부 해소** — 뮤테이션으로 독립 재현 확인(§11) |
| 잔여 | **N1·N2(문서화 권고, 코드 변경 불요)** — 아래 §11-2 |

**종료 판정: 로직 통과.** 계획된 인수조건을 모두 충족하고, 1라운드에서 지적한 테스트 공백(L1)·런타임 엣지케이스(L2)가
모두 코드+회귀 테스트로 닫혔다. 남은 N1·N2는 dev-notes 문장 보완 수준이라 **스프린트 진행을 막지 않는다.**
단 디바이스 스모크(§9)는 여전히 **사용자 판정 대기**이며, 이번 라운드에서 **S1이 최우선**이 됐다(사유 §11-2 N2).
...
| N1 — 멀티터치 동작이 plan §6과 달라짐 | `dev-rating-drag` | 문서화 권고(§11-2). 코드 변경 불요 |
| N2 — 신원 게이트 fallback 경로 무테스트 | `dev-rating-drag` | 문서화 권고 + 스모크 S1 최우선(§11-2). 코드 변경 불요 |
| L5 — 헤더 주석 경로 | (선택) | 선행 이월 nit |
...
값 튐이 사라지는 쪽이라 **동작으로는 개선**이므로 코드 수정은 불필요하고, dev-notes §6(알려진 제약)과 스모크 S7의 기대치를 이 동작에 맞춰 갱신하길 권고한다.
