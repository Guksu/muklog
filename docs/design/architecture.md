...
| `map-tab-nearby` (슬라이스2) | 일반 음식점 viewport 핀 — bbox + 디바운스(usePlaceSearch 패턴)로 Kakao Local(category FD6) 조회, saved/주변 핀 머지·구분 색, 비용 가드레일(전체조회 금지) 집중. 슬라이스1의 `initialRegion`/마커 계약 위에 증분. place-search Edge Function 확장(rect 파라미터) 또는 신규 nearby 함수. | #6 | 예정 |
