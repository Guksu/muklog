...
> **역할 경계**: 본 가이드는 **로직·통합·보안·비용·TDD·컨벤션**을 다룬다. **킷 시안 대비 비주얼 충실도(레이아웃·토큰·radius·폰트·카피)는 `visual-qa` 스킬 + `qa-visual` 에이전트 담당**이므로 여기서 다루지 않는다. 단, "토큰 경유(raw hex 0)" 같은 코드 규칙은 아래 "디자인/코드 품질"에서 코드 차원으로 본다(시안 대조 아님).
...
## 출력
`_workspace/{slug}/qa-report-logic.md` — 통과 / 실패(파일:라인+수정안) / 미검증(사유) 3분류를 **스펙 축**(plan/계약/인수조건)과 **컨벤션 축**(code-convention·토큰 규칙)의 별도 절로 나눠 보고한다(합산 순위 금지 — 한 축이 다른 축을 가리는 것을 방지). 커밋되지 않는 인계물이다(보존 기록은 리더가 `docs/history/`에 종합). 모든 인수조건 통과 전에는 스프린트를 "완료"로 표시하지 않는다.
